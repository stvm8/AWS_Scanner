...
"""
import boto3, re, json, os, datetime
from .privesc_detector import extract_allowed_actions
from .privesc_detector import analyze_policies
...
"""
Updated process_policies function for policy_reader.py
"""

def process_policies(policies, session, output_dir=None):
    """
    Process discovered policies - download, save, and analyze them with a focus on privilege escalation.
    ...
    # List to collect policy documents for privilege escalation analysis
    policies_with_documents = []
    ...
    for policy_info in policies:
        if len(policy_info) >= 5:
            ...
            if policy_document:
                ...
                # Add to the list of policies to analyze
                policy_tuple = list(policy_info)
                if len(policy_tuple) < 6:
                    policy_tuple.append(policy_document)
                policies_with_documents.append(policy_tuple)
                
                # Check if policy allows S3 bucket listing
                from .privesc_detector import extract_allowed_actions
                allowed_actions = extract_allowed_actions(policy_document)
                if 's3:ListBucket' in allowed_actions or 's3:ListAllMyBuckets' in allowed_actions or 's3:*' in allowed_actions or '*:*' in allowed_actions:
                    allow_s3_list_buckets = True
    
    # Analyze all policies for privilege escalation paths
    if policies_with_documents:
        print(f"\n{bold}{cyan}=== Privilege Escalation Analysis ==={reset}")
        
        # Import privilege escalation detector
        from .privesc_detector import analyze_policies
        
        # Analyze all policies
        analyze_policies(policies_with_documents)
    
    # Check if any policy allows S3 bucket listing and ask about enumeration
    if allow_s3_list_buckets:
        from .s3_bucket_enum import enumerate_single_bucket
        
        print(f"\nDetected S3 bucket listing permissions. Would you like to enumerate a specific bucket?")
        ...
        if choice.strip():
            ...
            # Ensure output location is valid, default to 'results' if needed
    ...
    print(f"\n{bold}{yellow}Note: Manually review all policies for additional security risks.{reset}")
